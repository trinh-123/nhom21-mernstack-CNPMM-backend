...
const client = new OAuth2Client(
  "75435593592-ibbekma2opi25sc4bnfnrr276ki2ne01.apps.googleusercontent.com"
);
//code here
//Get revenueByAdmin
...
module.exports.callbackFacebook = async (req, res) => {
  ...
  if (userByidFB == null && userByemail == null) {
    ...
    const set_pass = await User.findOneAndUpdate(
      { _id: user.id },
      { password: "1", groupid: 1 },
      {
        new: true,
      }
    );
...
module.exports.postContact = async (req, res) => {
  const Objectnew = await ContactModel.create({
    ...req.body,
    createdAt: Date(),
  });
  if (Objectnew) {
...
module.exports.postForgetPass = async (req, res) => {
  ...
  if (!userByEmail) {
    return res
      .status(202)
      .json({ msg: "Email không tồn tại!", success: false });
  }
  let password = text;
  ...
  transporter = nodemailer.createTransport(
    //"smtps://huynhnhan199999%40gmail:Huynhnhan999@smtp.gmail.com"
    {
      host: "smtp.gmail.com",
      port: 465,
      secure: true,
...
module.exports.postUpdateUser = async (req, res) => {
  ...
  const result = await User.findOneAndUpdate({ _id: req.body.id }, ObjUpdate, {
    new: true,
  });
  return res.status(201).json({
    success: true,
